...
export default class MXSerializer implements XmlSerializer {
  ...
  private static TRACE_SIZING: boolean = false;
  private static TRACE_ESCAPING: boolean = false;
  ...
  // element stack
  protected elNamespace: (string | null)[];
  protected elName: (string | null)[];
  protected elPrefix: (string | null)[];
  protected elNamespaceCount: (Int | null)[];

  // namespace stack
  protected namespaceEnd: Int = toInt(0);
  protected namespacePrefix: (string | null)[];
  ...
  private static BUF_LEN: Int = toInt(freemem() > 1000000 ? 8 * 1024 : 256);
  ...
  protected ensureElementsCapacity(): void {
    ...
    let iarr: (Int | null)[] = new Array<Int>(newSize);
    if (needsCopying) iarr = [...this.elNamespaceCount];
    else iarr[0] = toInt(0);
    this.elNamespaceCount = iarr;
  }

  protected ensureNamespacesCapacity() {
    // int size) {
    ...
    let newNamespacePrefix: (string | null)[] = new Array<string>(newSize);
    let newNamespaceUri: (string | null)[] = new Array<string>(newSize);
  ...
  public setProperty(name: string, value: object) {
    ...
    switch (name) {
      case this.PROPERTY_SERIALIZER_INDENTATION:
        this.indentationString = value.toString();
        break;
      case this.PROPERTY_SERIALIZER_LINE_SEPARATOR:
        this.lineSeparator = value.toString();
        break;
  ...
  public setOutputWithEncoding(writable: Writable, encoding: string) {
  ...
  public startDocument(encoding: string, standalone: boolean) {
    this.out?.cork();
    ...
    if (namespace != null) {
      if (namespace.length > 0) {
        ...
        if (
          ...
          let uri: string | null = this.namespaceUri[this.namespaceEnd - 1];
          if (uri == namespace) {
            let elPfx = this.namespacePrefix[this.namespaceEnd - 1];
            for (
              let pos = this.elNamespaceCount[this.depth - 1]! - 1;
              pos >= 2;
              --pos
            ) {
              let pf = this.namespacePrefix[pos];
              if (pf == elPfx) {
                let n = this.namespaceUri[pos];
                if (n == uri) {
                  --this.namespaceEnd;
                  prefix = elPfx;
                }
  ...
  public endTag(namespace: string, name: string): XmlSerializer {
    ...
    let startTagName: string | null = this.elName[this.depth];
    if (
    ...
    if (this.startTagIncomplete) {
      ...
      this.out?.write('</');
      let startTagPrefix: string | null = this.elPrefix[this.depth];
  ...
  public textBuf(buf: string[], start: Int, len: Int) {
    if (this.startTagIncomplete || this.setPrefixCalled) this.closeStartTag();
    if (this.doIndent && this.seenTag) this.seenTag = false;
  ...
  protected writeElementContent(txt: string, out: Writable) {
  ...
  protected static printable(s: string | null): string {
    ...
    let retval: string[] = new Array<string>(s.length + 2);
    retval[0] = "'";
  ...
  protected static addPrintable(retval: string[], ch: string, idx: Int) {
    switch (ch) {
      case '\b':
        retval[idx] = '\\b';
        break;
